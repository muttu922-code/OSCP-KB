...
**Prefer separate pages over one big file?** Open **`pages/index.html`** — a landing page with a button per section; each opens on its own page in a new tab (with its own search + nav). Same content as `OSCP-KB.html`, just split up. Regenerated by `build_html.py`.
